...
4. Check your permissions:
  ...
  Replace YOUR-GROUP-ID with the ID of your management group. Then re-execute the command to list subscriptions and confirm access.
...
5. Assign the custom role using a script:
  ...
  for SUBSCRIPTION_ID in $SUBSCRIPTIONS; do
      ...
      az account set --subscription "$SUBSCRIPTION_ID" # Assign the role to the principal for the current subscription
...
7. Run the script to assign this custom role to your app across all subscriptions under the management group:
