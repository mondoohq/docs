...
sidebar_position: 4
description: Add Mondoo annotations (similar to tags or labels) to the assets in your infrastructure
...
Don't confuse Mondoo annotations with metadata created and stored on the asset platform, such as AWS tags, Kubernetes labels, and so on. Annotations are metadata generated with Mondoo; labels and tags are metadata that Mondoo collects.
