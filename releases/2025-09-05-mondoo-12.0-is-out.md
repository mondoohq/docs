...
Automatically discover and scan all your subdomains with the new `networkdiscovery` provier for cnspec.
...
If scanning using inventories in cnquery and cnspec the previosly deprecated and hidden flags `--inventory-ansible` and `--inventory-domainlist` have been removed in favor of `--inventory-format-ansible` and `--inventory-format-domainlist`.
