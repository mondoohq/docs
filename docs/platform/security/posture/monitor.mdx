...
When Mondoo scans your assets, it produces a letter score to assess risk to your business. Mondoo scoring is based on policies that you choose that match the type of asset scanned. For example, you can choose policies for Windows, Kubernetes, Google Cloud, and so on. You [choose which policies are enabled](/platform/security/posture/policies/) in a space. An asset can match multiple policies enabled in a space.
