...
Each target that Mondoo scans receives a risk score that summarizes what percentage of the policy's checks the asset fails, weighted by each check’s impact. Risk scores are based on numeric scores between 0 and 100. These are the ranges for Mondoo risk scores:
