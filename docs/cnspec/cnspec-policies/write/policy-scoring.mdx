...
For these reasons, Mondoo recommends th banded scoring system to most of our customers.
...
More risk-averse companies can choose the decayed scoring system. It's more sensitive to all failures, especially to high and critical checks, but is slower to reflect improvements than the banded system.

The highest impact scoring system is useful only for the most risk-averse teams who can't abide any check failures. On the other end of the spectrum, the average scoring system provides a very positive representation of an asset's security, and at the same time may not motivate teams to improve.
...
When Mondoo first scans the asset based on the example policy:
...
Now suppose the team makes improvements to the asset so that it now passes another high-impact check and 3 more of the critical-impact checks. The asset:
...
Finally, the team fixes all but one critical issue. The asset:
