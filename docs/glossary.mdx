...
Each finding that Mondoo reveals has a [base score](/platform/security/posture/findings/#begin-with-a-base-score) associated with it. For misconfigurations exposed by Mondoo's security policies, the base score comes from the policy. For CVEs and advisories, the base score is the CVSS score.

### blast radius

The [blast radius](/platform/security/posture/findings/#blast-radius) of a finding is the impact that the finding has on a space. Mondoo can expose the same finding on multiple assets in a space. It calculates blast radius of the finding using the risk scores of all of the assets in the space that have that finding.
