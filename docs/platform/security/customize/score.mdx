...
Each target that Mondoo scans receives a risk score that summarizes what percentage of the policy's [checks](/cnspec/cnspec-policies/write/simple/#checks) the asset passes. Risk scores are based on numeric scores between 0 and 100. These are the ranges for Mondoo risk scores:
