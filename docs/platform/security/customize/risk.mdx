...
4. Under **High-priority asset** or **Low-priority asset**, select the plus (+) button to add a condition.

  ![Define assets as high or low risk based on metadata in Mondoo](/img/platform/security/add-first-condition.png)

5. Choose whether to assign priority based on a cloud tag/label or a Mondoo annotation.

6. Select the plus (+) button to the right and choose the metadata key for the condition. To be flagged high or low priority, an asset must have a tag, label, or annotation with this key.

   ![Choose the key](/img/platform/security/choose-key.png)

7. Choose the value for the condition. To be flagged high or low priority, an asset must have a tag, label, or annotation with this key value.
...
8. To add another condition, select **+ ADD CONDITION** and follow steps 6-8 again. If you define multiple conditions, an asset only must meet one (or more) of the conditions to be flagged high or low priority. (They're combined with a Boolean OR.)

   ![Add another condition](/img/platform/security/another-condition.png)

9. Select the **SAVE CHANGES** button. Changes take place immediately.
