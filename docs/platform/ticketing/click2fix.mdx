...
import click2fixDiagram from "/img/platform/ticketing/click2fix-diagram.png";
import click2fixRepo from "/img/platform/ticketing/click2fix-repo-setup.png";
import click2fixPR from "/img/platform/ticketing/click2fix-pr.png";
import click2fixPRdetails from "/img/platform/ticketing/click2fix-pr-details.png";
...
<img src={click2fixDiagram} width="700" />
...
<img src={click2fixRepo} width="700" />
...
<img src={click2fixPRdetails} width="700" />




### GitHub + Ansible automation

As a first step, we implement the complete flow for Ansible + GitHub. This allows us to focus on a specific use case and test it out with customers early.

**User Journey in the Console**

- Ansible integration
    - Instructions to create GitHub repo
    - Assign new GitHub repo via GitHub App, we browse the list of GitHub repos
    - Do you want to generate the SSH Key
        - Yes ⇒ Create ssh private and public key
        - No ⇒ User uploads private key
    - Show instruction how users can add the ssh key to windows and linux, show powershell + bash snippets
    - Question if user wants to initialize the git repo
        - generate repo structure
        - generate inventory.yaml
        - setup GitHub actions to run Ansible and use the ssh key, if permissions are missing show instructions to the user
- Create PR from Findings page
    - Read assigned package score and generate the ansible task for the playbook
