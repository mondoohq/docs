...
2. To automatically discover all Linux and Windows virtual machines (VMs) in your subscription and scan them using Azure Run Command, enable **Scan virtual machines**. This requires an Azure user account with the (Global Administrator role)(https://learn.microsoft.com/en-us/azure/active-directory/roles/permissions-reference#global-administrator).
...
4. Under **Copy the installation command**, you see a custom command that Mondoo generates for you based on your selections above. Running this command in Azure Cloud Shell creates the Mondoo Azure integration for you.
   ...
   By default, the name for the integration (visible in the Mondoo console and in reports) is `new-azure-integration`. You can change this name now in the command or change it any time after you create the integration.
...
7. Paste the copied command in Azure Cloud Shell. Respond to these prompts:
   ...
   - When the Mondoo automation resports success, you're finished in Azure Cloud Shell.
