...
Mondoo scores and ranks security findings to help you prioritize fixes to your infrastructure. Our unique algorithms calculates score and rank based on a combination of contextual risk factors in your environment. You can spend more time closing security gaps and less time sifting through irrelevant or low-priority findings.
...
Mondoo assigns a risk score of Low, Medium, High, or Critical to each finding. The risk score is calculated from using a base score and contextual risks.
...
If Mondoo discovers a finding on many assets in a space and the risk scores of that finding on those assets is high, then the blast radius is high. If Mondoo discovers a finding on many assets but the risk score of the finding on those assets is low, then the blast radius is lower.
