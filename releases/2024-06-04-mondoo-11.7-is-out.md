...
Using this provider you can create custom security policies to enforce your organizational standards in CI jobs:
...
Further extend your discovery of IaC assets in your GitHub and GitLab repositories or projects with Kubernetes manifest discovery. With one command Mondoo will track down all your manifest files, no matter where they're hiding.
...
### Scan and query SBOM files directly

Query SBOM file content directly with cnquery as if the files are real running assets:
...
- Add `permissionGrantPoliciesAssigned` to the `defaultUserRolePermissions` field
...
- Discover all resources when scanning K8s manifests
- Fix incorrect asset names when scanning K8s manifests without namespaces
...
- Don't fail discovery in a single VMware ESXi host cannot be reached.
- Remove non-functional sorting by risk factors in tables.
- Add a "Type" column in search when filtering by "All" so it's more clear if entries are assets, checks, or CVEs.
