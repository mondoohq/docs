...
5. Select **Create** and select **Continue**.

6. Select **Done** > **Save**.

7. At the top, select **Key** > **Add Key** > **Create new key**.

8. Make sure the key type is set to **JSON**, then select **Create**. You see a message that the service account's private key JSON file was downloaded to your computer. Note the file name and where your browser saves it. You need it for later steops.

9. Select **Close**.

   ![Google Service Account Client ID](/img/platform/infra/saas/gw/gw-service-account-id.png)

10. Record the **OAuth 2 / Client ID** from the list of service accounts. You need that for the next step.
